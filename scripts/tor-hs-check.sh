...
function curl_test {
echo "Checking HSv2"
if torsocks curl --max-time 30 "${HSV2}":5222/ | grep "xml" | grep "stream" > /dev/null
  then
    echo "The Tor HSv2 "${HSV2}":5222 was reachable!" > /dev/null
  else
    echo "The Tor HSv2 "${HSV2}":5222 is unreachable!" | mail -s "Tor HSv2 Down" cron@lunar.systems
fi

echo "Checking HSv3"
if torsocks curl --max-time 30 "${HSV3}":5222/ | grep "xml" | grep "stream" > /dev/null
  then
    echo "The Tor HSv3 "${HSV3}":5222 is reachable!" > /dev/null
  else
