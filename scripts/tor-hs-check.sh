...
if torsocks curl --connect-timeout 60 y2qmqomqpszzryei.onion:5222/ | grep "xml" | grep "stream" > /dev/null
  then
...
if torsocks curl --connect-timeout 60 6voaf7iamjpufgwoulypzwwecsm2nu7j5jpgadav2rfqixmpl4d65kid.onion:5222/ | grep "xml" | grep "stream" > /dev/null
  then
